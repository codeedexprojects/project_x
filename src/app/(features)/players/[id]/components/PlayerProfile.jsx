...
import { Trophy, Edit, Save, X, Download, User, Camera } from 'lucide-react';
...
export default function PlayerProfile({ 
  ...
  const getRankColor = (totalPoints) => {
    if (totalPoints >= 100) return "bg-gradient-to-r from-green-500 to-green-600";
    if (totalPoints >= 50) return "bg-gradient-to-r from-blue-500 to-blue-600";
    if (totalPoints >= 25) return "bg-gradient-to-r from-purple-500 to-purple-600";
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Manage player information and tournament history</p>
            </div>
          </div>
              ...
              <div className="flex flex-col items-center gap-4 mb-6">
                ...
                {isEditing ? (
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-500">Player ID: {playerData.playerId}</p>
                  </div>
                )}
              </div>
                  ...
                  <div className="bg-gradient-to-r from-indigo-50 to-white p-4 sm:p-6 border-b border-gray-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 capitalize">{tournament.type} • {tournament.matches.length} tournaments</p>
                      </div>
                    </div>
                    <span className={`${getRankColor(tournament.totalPoints)} text-white text-xs font-semibold px-3 py-2 rounded-lg`}>
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {tournament.matches.map((match, matchIdx) => (
                          <tr key={matchIdx} className="hover:bg-gray-50 transition-colors">
                            <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{match.date}</td>
                            <td className="px-4 py-3 text-sm text-gray-800">{match.tournament}</td>
                            <td className="px-4 py-3 text-sm text-gray-600">{match.location}</td>
                            <td className="px-4 py-3">
                              <span className={`${getPositionColor(match.color)} text-xs font-medium px-3 py-1 rounded-full inline-block min-w-[100px] text-center`}>
                                {match.position}
                              </span>
                            </td>
